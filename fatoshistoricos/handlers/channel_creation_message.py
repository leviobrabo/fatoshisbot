...
def enviar_mensagem_aniversario(CHANNEL):
    try:
        ...
        if (
            data_atual.month == data_criacao.month
            and data_atual.day == data_criacao.day
        ):
...
def agendar_aniversario():
    while True:
        ...
        proximo_aniversario = datetime(
            agora.year, data_criacao.month, data_criacao.day, 0, 0, 0
        )

        if agora >= proximo_aniversario:
            proximo_aniversario = datetime(
                agora.year + 1, data_criacao.month, data_criacao.day, 0, 0, 0
            )
